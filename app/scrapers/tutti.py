...
class TuttiScraper:
    ...
    async def __aenter__(self):
        ...
        try:
            ...
            scraping_logger.info("[Tutti] Playwright disponible, mode JavaScript activé")
        except (ImportError, Exception) as e:
            # Fallback sur aiohttp
            scraping_logger.warning(f"[Tutti] Playwright non disponible ({e}), mode aiohttp activé")
            ...
            try:
                async with self._session.get(url) as response:
                    if response.status != 200:
                        raise TuttiError(f"HTTP {response.status}", status_code=response.status)
                    return await response.text()
            except aiohttp.ClientError as e:
                raise TuttiError(f"Erreur réseau: {e}")
